...
function createRecipeCard(recipe) {
    ...
    recipe_card.style.backgroundImage = `url(${recipe.image})`
...
function handleChangeRecipe() {
    ...
    recipe_cards.forEach(recipe_card => {
        recipe_card.addEventListener('click', () => {
            ...
            const { image, title, description, ingredients, instructions } = getRecipeById(id - 1);
                    ...
                    <div class="recipe__content">
                        ...
                        <div class="recipe__ingredients">
                            ...
                            <ul class="recipe__list">
                                ${ingredients.map(ingredient => `<li><span>✓</span>${ingredient}</li>`).join('')}
                            </ul>
                        </div>
                        <div class="recipe__instructions">
                            <h4>Preparación</h4>
                            <ul class="recipe__list">
                             ${instructions.map((instruction, index) => `<li><span>${index + 1}.</span> ${instruction}</li>`).join('')}
                            </ul>
                        </div>
                    </div>
                </div>
